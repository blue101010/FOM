...
resolution-technique page (SCHEMA_V3 §3.5).

ATT&CK is an *anchor* carried per entry, never the definition of an entry: this
script promotes a curated ID -> ATT&CK mapping onto each entry page so the
related techniques are displayed where the reader lands.

It does NOT touch the domain pages under `tactics/`. Those are generated by
`v3/render_taxonomy_docs.py`, which reads these tables back to build the `ATT&CK`
orientation column. Two writers for one file is the drift v3 exists to remove.

Usage:
    python v3/attach_attack_related.py            # dry run (default)
    python v3/attach_attack_related.py --write    # apply to the corpus
"""
...
# (the former TACTIC_SENTENCE_RE anchored a pointer into the domain pages; those pages
# are generated by v3/render_taxonomy_docs.py now, so this script no longer writes them)
...
POINTER = ("\n\n> Per-entry related ATT&CK IDs are rendered on every technique and\n"
           "> counter-technique page as a `Related MITRE ATT&CK` table (SCHEMA_V3 §3.5).\n")
...
def main() -> None:
    ...
    print(f"{mode}   techniques={len(techniques)} counters={len(counters)} "
          f"changed={len(changed)} (domain pages are generated, not touched here)")

...
Each entry below is a curated pair (SCHEMA_V3 §3.5 per-entry ATT&CK table
included). The script:
...
Run:  python v3/create_todo_entries.py
Then: python v3/render_taxonomy_docs.py --write
...
def make_tactic_page(cat: str, htb: str = "HTB: Misc", description: str = "") -> str:
    """Minimal editorial skeleton for a new domain page.

    Only the Description is editorial. The positioning boilerplate, the pair table
    and the counts are written by `v3/render_taxonomy_docs.py`, which owns these
    pages; emitting them here would duplicate the source of truth and let the two
    drift. Run the renderer immediately after creating the file.
    """
    body = description or (
        "Describe the subject of this domain: what the author conceals, and in what."
    )
    return (
        f"# {cat} — {TACTIC_NAMES[cat]}\n\n"
        f"> **Domain ID:** `CTFT-TA-{cat}`  \n"
        f"> **HTB mapping:** {htb}  \n"
        f"> **Techniques:** 0\n\n"
        "## Description\n\n"
        f"{body}\n\n"

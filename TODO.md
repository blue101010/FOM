# TODO — Suggested techniques & counter-techniques inferred from `C:\DATA\CTF\ALL`

> Source of inference: local CTF notes under `C:\DATA\CTF\ALL` (508 files, ~50 topic folders).
...
## A. Populate `HWR` (Hardware) — currently reserved, empty

| ID | Technique (hide/design) | Counter-technique (recover) | Inferred from | ATT&CK anchor |
| --- | --- | --- | --- | --- |
| HWR-001 | RF/SDR signal embedding | Demodulate and decode RF captures (Gqrx, GnuRadio, Universal Radio Hacker) | `sdr_RF/`, `audio/rf_signal.wav` | T1001.002 |
| HWR-002 | USB HID keystroke concealment | Reconstruct keystrokes from captured USB HID traffic | `usb/` (scan codes, HID tables) | T1056.001 |
| HWR-003 | Badge / embedded-device firmware concealment | Dump and reverse badge firmware | `badge/` | T1027 |

## B. New tactic `NET` (Network) — currently folded into FPN/WEB
...
## D. Reverse Engineering (`REV`, next free: 006)
...
## E. Binary Exploitation (`PWN`, next free: 006)
...
## F. Cryptography (`CRY`, next free: 008)
...
## G. Steganography (`STE`, next free: 010)
...
## H. Web Exploitation (`WEB`, next free: 010)
...
## L. Full Pwn / Multi-Stage (`FPN`, next free: 008)
...
## M. Game / Protocol (`GAM`, next free: 006) · Coding (`COD`, next free: 006) · Jail (`JAL`, next free: 006)
...
- Consider one `WIFI` / `SDR` split if `HWR` fills up; keep ATT&CK anchors per-entry, not per-tactic.
